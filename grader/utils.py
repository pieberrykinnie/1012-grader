...
def analyze_code(script_path: str) -> CodeAnalysis:
    ...
    with open(script_path, 'r') as file:
        try:
            ...
            for func_name, return_count in visitor.functions_with_multiple_returns.items():
                line_issues.append(
                    LineIssue(
...
def generate_report(
    ...
    if execution_result.timeout_occurred:
        ...
        report_lines.append(f"[!] Script exited with error (return code: {execution_result.return_code})")
    ...
    # Summary
    report_lines.append("\nSummary:")
    report_lines.append(f"- Output Validation: {len(output_validation.found_patterns)}/{output_validation.total_patterns} checks passed")
